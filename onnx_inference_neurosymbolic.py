...
class ONNXNeuroSymbolicInference:
    ...
    def _initialize_scallop(self):
        ...
        scallop_program = '''
        // Landing Safety Knowledge Base for Stage 2/3 Models
        
        // Define safe and hazardous surfaces based on our 6-class model
        rel safe_surface = {
            "road", "vegetation", "roof"  // Generally safe for landing
        }
        
        rel hazardous_surface = {
            "vehicle", "facade"  // Dangerous for landing
        }
        
        rel uncertain_surface = {
            "other"  // Requires additional analysis
        }
        
        // Weather conditions affecting landing safety
        ...
        rel high_confidence_prediction(x, y) =
            prediction_confidence(x, y, conf) and
            conf > 0.7
        
        // Emergency landing assessment
        rel emergency_landing_possible(x, y, risk_level) = 
            suitable_landing_zone(x, y, _) and
            risk_level := "low"
        ...
        rel emergency_landing_possible(x, y, risk_level) = 
            ...
            not nearby_hazardous(x, y) and
            risk_level := "high"
        
        rel nearby_hazardous(x, y) = 
            nearby_surface(x, y, surface) and
        ...
        rel prediction_reliable(reliable) = 
            avg_uncertainty(unc) and
            unc < 0.3 and
            reliable := true
        ...
        rel landing_recommendation(action) = 
            mission_safety_level("safe") and
            prediction_reliable(true) and
            action := "proceed_landing"
        
        rel landing_recommendation(action) = 
            mission_safety_level("caution") and
            prediction_reliable(true) and
            action := "caution_landing"
        
        rel landing_recommendation(action) = 
            mission_safety_level("danger") or
            prediction_reliable(false) and
    ...
    def visualize_results(self, image: np.ndarray, result: Dict, save_path: Optional[str] = None) -> np.ndarray:
        ...
        if reasoning.get('reasoning_enabled', False):
            ...
            if reliable:
                is_reliable = reliable[0][0] if reliable[0] else False
